...
interface TopRatedProps {
    showButtonsTR: boolean;
    showListTypeTR: boolean;
    showListTypeTRF: boolean;
}

const TopRated: React.FC<TopRatedProps> = ({ showButtonsTR, showListTypeTR, showListTypeTRF }) => {
        ...
        <div className='block pl-7'>
            <div className="flex flex-wrap -mx-4">
                <div className="relative w-full px-4 md:w-9/12">
                    <h2 className="text-2xl font-semibold text-gray-800 mt-5 text-left ml-7 uppercase">Top Rated</h2>
                </div>
                {showButtonsTR && (
                    <div className="relative w-full px-4 md:w-3/12">
                        <button onClick={sortByName} className={`bg-blue-600 text-white px-2 py-1 text-xs font-medium rounded transition-all duration-300 hover:bg-blue-700 mr-2 mb-2 cursor-pointer ${activeButton === 'name' ? 'bg-orange-500' : ''}`}>
                        ...
                        <button onClick={sortByRating} className={`bg-blue-600 text-white px-2 py-1 text-xs font-medium rounded transition-all duration-300 hover:bg-blue-700 mr-2 mb-2 cursor-pointer ${activeButton === 'rating' ? 'bg-orange-500' : ''}`}>
